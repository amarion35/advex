...
class Classifier(nn.Module):
    ...
    def forward(self, x, training):
        ...
        x = self.conv1(x)
        x = F.relu(x)
        x = F.dropout2d(x, 0.1, training=training)
        x = self.conv2(x)
        x = F.relu(x)
...
def demo():
    ...
    # Train classifier
    classifier = Classifier(input_shape=784)
    classif_model = Model(classifier, device='cuda:2')
    optimizer = optim.Adam(classifier.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-8)
    loss = nn.CrossEntropyLoss(reduction='mean')
    metrics = [sparse_categorical_accuracy]
    classif_model.compile(optimizer, loss, metrics=metrics)
    classif_model.fit(X_train, Y_train, batch_size=128, epochs=25, validation_data=(X_test, Y_test))
